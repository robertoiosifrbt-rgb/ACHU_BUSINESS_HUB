...
export class VisualBusinessGame extends Business4XGame{
 ...
 async start(){
  ...
  this.scene.background.setHex(0xb8cbc3);if(this.scene.fog){this.scene.fog.color.setHex(0xb8cbc3);this.scene.fog.density=.0065}
 ...
 addWorkers(){
  ...
  for(let i=0;i<9;i++){
   const view=this.assets?.cloneCharacter?.()||simpleWorker(accents[i%accents.length]);view.position.set(0,0,0);const groundOffset=fitHeight(view,.82+(i%3)*.04)
   ...
   this.workers.push({view,mixer,curve,offset:(i*.117)%1,speed:.0000105+(i%5)*.0000012,groundOffset})
  }
 }
 animateWorkers(t){
  const dt=Math.min(.05,Math.max(0,(t-(this.workerAnimAt||t))/1000));this.workerAnimAt=t
  for(const w of(this.workers??[])){w.mixer?.update(dt);const u=(w.offset+t*w.speed)%1,p=w.curve.getPointAt(u),tan=w.curve.getTangentAt(u);w.view.position.set(p.x,.025+w.groundOffset,p.z);w.view.rotation.y=Math.atan2(tan.x,tan.z)}
  animateCampusTraffic(this.cityTraffic,t);this.worldOverlay?.animate(t)
 }
 resize(){
  const w=innerWidth,h=innerHeight,aspect=w/h,view=this.mode==='world'?14.6:12.1;this.camera.left=-view*aspect;this.camera.right=view*aspect;this.camera.top=view;this.camera.bottom=-view;this.camera.updateProjectionMatrix();this.renderer?.setSize(w,h,false)
 }
 setMode(mode){
 ...
 focusWorldObjective(announce=true){
  if(!this.worldOverlay)return;this.worldOverlay.sync(this.state);const p=this.worldOverlay.objectivePoint();this.controls.target.copy(p);this.camera.position.set(p.x+13.5,18.5,p.z+13.5);this.camera.lookAt(p);this.camera.zoom=1;this.camera.updateProjectionMatrix();if(announce)this.ui?.toast('Follow the gold NEXT marker')
 }
 focusBuilding(id){super.focusBuilding(id);this.camera.position.y=14.5;this.camera.updateProjectionMatrix()}
 rebuildCity(){super.rebuildCity();this.cityTraffic=decorateCampus(this.city)}
 update(){
  super.update();const now=performance.now();if(this.worldOverlay&&now-this.visualSyncAt>500){this.visualSyncAt=now;this.worldOverlay.sync(this.state)}
 }
}
