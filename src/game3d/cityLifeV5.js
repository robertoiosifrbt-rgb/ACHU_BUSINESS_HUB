...
function fitVehicle(view,targetLength=1.45){
 ...
 // The supplied FBX cars are long on local X. Preserve that model-axis
 // correction instead of overwriting it every animation frame.
...
function buildStreetCells(){
 ...
 // Outer distributor ring: serves the buildings at ±9.6.
 for(let i=-4;i<=4;i++){add(-4,i);add(4,i);add(i,-4);add(i,4)}
 // Inner campus ring: serves the four core blocks without turning every
 // crossing into a four-way junction.
 for(let i=-2;i<=2;i++){add(-2,i);add(2,i);add(i,-2);add(i,2)}
 // One deliberate north/south and east/west spine connects both rings.
...
function roadSpec(c){
 ...
 if(n===3){
  // road-intersection default: N+E+W (missing S)
  if(!has('S'))return['roadIntersection',0]
  if(!has('W'))return['roadIntersection',Math.PI/2]
  if(!has('N'))return['roadIntersection',Math.PI]
  return['roadIntersection',-Math.PI/2]
 }
 if(n===2){
  if(has('E')&&has('W'))return['roadStraight',0]
  if(has('N')&&has('S'))return['roadStraight',Math.PI/2]
  // road-bend default: N+W
 ...
 if(n===1){
  // road-end default opening: E
  if(has('E'))return['roadEnd',0]
  if(has('S'))return['roadEnd',Math.PI/2]
  if(has('W'))return['roadEnd',Math.PI]
  return['roadEnd',-Math.PI/2]
 }
 return['roadStraight',0]
}
function buildRoadNetwork(parent,bank){
 const cells=buildStreetCells()
 for(const k of cells){const [x,z]=k.split(',').map(Number),spec=roadSpec(connections(cells,x,z));roadTile(parent,bank,spec[0],x,z,spec[1])}
 return cells
}
function curbLight(parent,bank,x,z,rot=0){
 const v=bank?.models?.roadLight?bank.clone('roadLight'):null;if(!v)return
...
function roundedLoop(min,max,r=.55){
 const p=new THREE.CurvePath(),v=(x,z)=>new THREE.Vector3(x,0,z)
 p.add(new THREE.LineCurve3(v(min+r,min),v(max-r,min)))
 p.add(new THREE.QuadraticBezierCurve3(v(max-r,min),v(max,min),v(max,min+r)))
 p.add(new THREE.LineCurve3(v(max,min+r),v(max,max-r)))
 p.add(new THREE.QuadraticBezierCurve3(v(max,max-r),v(max,max),v(max-r,max)))
 p.add(new THREE.LineCurve3(v(max-r,max),v(min+r,max)))
 p.add(new THREE.QuadraticBezierCurve3(v(min+r,max),v(min,max),v(min,max-r)))
 p.add(new THREE.LineCurve3(v(min,max-r),v(min,min+r)))
 p.add(new THREE.QuadraticBezierCurve3(v(min,min+r),v(min,min),v(min+r,min)))
 return p
}

export const CAMPUS_WALK_ROUTES=[
 path([[-4.95,-3.2],[-4.95,3.2]]),path([[-1.25,-3.2],[-1.25,3.2]]),path([[1.25,-3.2],[1.25,3.2]]),path([[4.95,-3.2],[4.95,3.2]]),
 path([[-9.6,-4.95],[-3.2,-4.95]]),path([[3.2,4.95],[9.6,4.95]]),path([[-9.6,1.25],[-3.2,1.25]]),path([[3.2,-1.25],[9.6,-1.25]])
]
...
export function decorateCampus(root,bank){
 ...
 const traffic=[
  {view:vehicle(g,bank,0,1.48),curve:innerA,offset:.04,speed:.0000105},
  {view:vehicle(g,bank,1,1.58),curve:outerA,offset:.38,speed:.0000086},
  {view:vehicle(g,bank,2,1.5),curve:innerB,offset:.71,speed:.0000095},
  {view:vehicle(g,bank,3,1.42),curve:outerB,offset:.55,speed:.0000088},
  {view:vehicle(g,bank,4,1.5),curve:innerA,offset:.82,speed:.0000082}
 ]
