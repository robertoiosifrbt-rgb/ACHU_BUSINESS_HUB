...
const files={
 ...
 scaffold:'scaffolding-structure.glb',barrier:'detail-barrier-strong-type-a.glb',dumpster:'detail-dumpster-closed.glb',
 pineLarge:'tree-pine-large.glb',pineSmall:'tree-pine-small.glb',truckGreen:'truck-green.glb',truckGrey:'truck-grey.glb'
}
...
function placeholder(name){
 ...
 if(n.includes('pine')){
  ...
  const cab=new THREE.Mesh(new THREE.BoxGeometry(.48,.55,.62),new THREE.MeshStandardMaterial({color:0x40545a,roughness:.9}));cab.position.set(.62,.42,0);g.add(cab)
...
export class AssetBank{
 ...
 async load(){
  ...
  results.forEach((result,i)=>{
   ...
   else console.warn(`Kenney asset failed: ${file}`,result.reason)
  })
  this.ready=true
  return this
 }
