...
function park(bank,parent,x,z,r=2.3){disk(parent,r,0x73977b,1,.006);for(let i=0;i<7;i++){const a=i*2.399,rr=r*.3+(i%3)*.48;asset(bank,parent,i%3===0?'downtownTreeA':'pineSmall',x+Math.cos(a)*rr,0,z+Math.sin(a)*rr,a,i%3===0?.16:.25)}}
...
function importantOpportunity(bank,parent,resource,x,y,featured){const colors={meat:0x55dfa7,wood:0x65b9ff,coal:0xffc65d,iron:0xce9eff},c=colors[resource]??0x8bcfc0;smallOffice(bank,parent,x,y);if(featured){ring(parent,.68,c,.34);const m=glowPin(c);m.position.set(.34,1.02,.1);parent.add(m);if(hash(x,y)>.58)asset(bank,parent,hash(x,y)>.78?'truckGreen':'truckGrey',-.3,.02,.35,.25,.14)}}
 ...
 build(){
  ...
  for(let y=0;y<SIZE;y++)for(let x=0;x<SIZE;x++){
   ...
   if(scouted){
    ...
    else if(hash(x,y)>.7)smallOffice(this.bank,c,x,y)
