...
export class Business4XGame extends Retro4XGame{
 ...
 async start(){
  ...
  this.camera.zoom=1;this.resize();this.ui.refresh();this.audio.prepare()
 ...
 addWorkers(){
  ...
  routes.forEach((r,i)=>{
   ...
   let mixer=null
   if(view.userData.realCrew&&clip){mixer=new THREE.AnimationMixer(view);const action=mixer.clipAction(clip);action.timeScale=.78+(i%3)*.06;action.play();mixer.update(i*.1)}
 ...
 companyCapability(){return super.armyPower()}
 setMode(mode){
 ...
 resetGame(){this.audio?.stopAmbient();this.state=resetState();location.reload()}
 ...
 tickMarches(now){
  ...
  for(const target of completed){
   const d=worldTile(...parseTile(target)),rep=d.reviewReward??Math.max(1,Math.round((d.quote??d.yield??0)/40))
   this.state.resources.coal=(this.state.resources.coal??0)+rep;this.state.power+=rep*2
   this.ui?.toast(`Job complete · £${Math.round(d.quote??d.yield??0)} revenue · +${rep} Reputation`)
  }
  return changed||completed.length>0
 }
 finishOutbound(m,now){
  const type=m.type;super.finishOutbound(m,now)
  if(type==='attack'){const won=this.state.world.battleReports?.[0]?.win;won?this.audio.success():this.audio.fail()}
