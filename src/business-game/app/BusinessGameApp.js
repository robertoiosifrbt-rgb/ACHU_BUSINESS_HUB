...
export class BusinessGameApp{
 ...
 start(){
  this.host.innerHTML='';this.host.className='business-game';this.store=new BusinessStore();this.scene=new BusinessScene(this.host);this.ui=new GameShell(this.host,this.store);this.unsubscribe=this.store.subscribe(state=>this.scene.update(state));return this
 }
