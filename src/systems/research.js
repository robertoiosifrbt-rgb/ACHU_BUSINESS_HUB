...
export function startResearch(state,id){
 ...
 if((state.buildings.researchCenter??0)<1)return{ok:false,error:'Build Research Center first'}
 ...
 spend(state.resources,cost);const speed=1+researchBonus(state,'researchSpeed');const ms=def.seconds*Math.pow(1.5,target-1)*1000/DEMO_SPEED/speed
