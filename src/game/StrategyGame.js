...
export class StrategyGame{
 ...
 focusBuilding(id){const p=this.city.getBuildingPosition(id);if(!p)return;this.clearSelection();const z=this.world.scale.x;this.world.position.set(window.innerWidth/2-p.x*z,window.innerHeight/2+80-p.y*z);this.city.focus(id);this.toast(`${BUILDINGS[id].name} located`)}
