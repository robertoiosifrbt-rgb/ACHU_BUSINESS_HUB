...
export class StrategyGame{
 ...
 async start(){
  this.applyOfflineProduction();this.app=new Application();await this.app.init({resizeTo:window,antialias:true,background:0xb8dce7,resolution:Math.min(devicePixelRatio,2),autoDensity:true});this.mount.appendChild(this.app.canvas);await loadVisualAssets()
 ...
 focusBuilding(id){const d=BUILDINGS[id];if(!d)return;this.hud.detail.visible=false;this.hud.researchPanel.visible=false;const z=this.world.scale.x;this.world.position.set(window.innerWidth/2-d.x*z,window.innerHeight/2+100-d.y*z);this.city.focus(id);this.toast(`${d.name} located`)}
