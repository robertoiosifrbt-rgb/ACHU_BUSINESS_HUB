...
export class Hud extends Container{
 ...
 makeStatic(){
  ...
  this.queueBox=panel(238,48,.86,0x0b2834,14);this.addChild(this.queueBox);this.queue=text('',9,0xffffff,'800');this.queue.position.set(11,7);this.queueBox.addChild(this.queue)
 ...
 refresh(){const prod=resourceProduction(this.game.state),all=1+researchBonus(this.game.state,'allOutput');for(const k of Object.keys(this.resourceTexts)){const bonus=1+researchBonus(this.game.state,`${k}Output`);this.resourceTexts[k].text=fmtShort(this.game.state.resources[k]??0);this.rateTexts[k].text=`+${(prod[k]*all*bonus).toFixed(1)}/s`}this.power.text=this.game.mode==='world'?`⚔ ${fmt(this.game.armyPower())}`:`⚡ ${fmt(this.game.state.power)}`;const c=this.game.state.construction,r=this.game.state.researchJob,w=this.game.state.world;this.queue.text=this.game.mode==='world'?`◎ Territory ${w.owned.length} · March ${w.marches.length}/${this.game.marchCapacity()}\n⚔ ${w.defeated.length} camps · ${w.trainingJob?'Training':'Army ready'}`:`🔨 ${c?`${BUILDINGS[c.id].name} ${c.target} · ${fmtTime(c.finishAt-Date.now())}`:'Builder idle'}\n⌬ ${r?`${findResearch(r.id)?.name??r.id} ${r.target} · ${fmtTime(r.finishAt-Date.now())}`:'Research idle'}`;this.quest.refresh();this.armyPanel.refresh()}
