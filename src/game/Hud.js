...
export class Hud extends Container{
 ...
 makeStatic(){
  ...
  this.cityBtn=roundButton('⌂','CITY',0x315d70);this.cityBtn.on('pointertap',()=>this.game.showCity());this.worldBtn=roundButton('◎','WORLD',0x6b7040);this.worldBtn.on('pointertap',()=>this.game.showWorld());this.researchBtn=roundButton('⌬','RESEARCH',0x347fa5);this.researchBtn.on('pointertap',()=>this.openResearch());this.helpBtn=roundButton('⚑','ALLIANCE',0x4f8467);this.helpBtn.on('pointertap',()=>this.game.helpConstruction());this.moreBtn=roundButton('⋯','MORE',0x566772);this.addChild(this.cityBtn,this.worldBtn,this.researchBtn,this.helpBtn,this.moreBtn)
 }
 layout(){const w=window.innerWidth,h=window.innerHeight;this.profile.position.set(10,10);const scale=Math.min(1,(w-20)/368);this.top.scale.set(scale);this.top.position.set(Math.max(10,w-368*scale-10),10);const qs=Math.min(1,(w-20)/350);this.quest.scale.set(qs);this.quest.position.set(10,68);this.queueBox.position.set(10,h-105);const nav=[this.cityBtn,this.worldBtn,this.researchBtn,this.helpBtn,this.moreBtn],gap=Math.min(74,(w-24)/5),start=w/2-gap*2;nav.forEach((b,i)=>b.position.set(start+i*gap,h-43));if(this.detail.visible)this.detail.position.set(Math.max(10,(w-this.detail.sheetW)/2),h-this.detail.sheetH-74);if(this.researchPanel.visible)this.researchPanel.position.set(Math.max(10,(w-this.researchPanel.sheetW)/2),h-this.researchPanel.sheetH-74);if(this.worldPanel.visible)this.worldPanel.position.set(Math.max(10,(w-this.worldPanel.sheetW)/2),h-this.worldPanel.sheetH-74)}
 refresh(){const prod=resourceProduction(this.game.state),all=1+researchBonus(this.game.state,'allOutput');for(const k of Object.keys(this.resourceTexts)){const bonus=1+researchBonus(this.game.state,`${k}Output`);this.resourceTexts[k].text=fmtShort(this.game.state.resources[k]??0);this.rateTexts[k].text=`+${(prod[k]*all*bonus).toFixed(1)}/s`}this.power.text=this.game.mode==='world'?`⚔ ${fmt(this.game.armyPower())}`:`⚡ ${fmt(this.game.state.power)}`;const c=this.game.state.construction,r=this.game.state.researchJob;this.queue.text=this.game.mode==='world'?`◎ ${this.game.state.world.owned.length} territories\n⚔ ${this.game.state.world.defeated.length} camps cleared`:`🔨 ${c?`${BUILDINGS[c.id].name} ${c.target} · ${fmtTime(c.finishAt-Date.now())}`:'Builder idle'}\n⌬ ${r?`${findResearch(r.id)?.name??r.id} ${r.target} · ${fmtTime(r.finishAt-Date.now())}`:'Research idle'}`;this.quest.refresh()}
 showBuilding(id){this.researchPanel.visible=false;this.worldPanel.visible=false;this.detail.open(id);this.layout()}
 showWorldTile(id){this.detail.visible=false;this.researchPanel.visible=false;this.worldPanel.open(id);this.layout()}
 openResearch(){this.detail.visible=false;this.worldPanel.visible=false;this.researchPanel.open();this.layout()}
}
