...
export class Hud extends Container{
 ...
 showBuilding(id){
  this.researchPanel.visible=false;this.detail.removeChildren();this.detail.visible=true;const d=BUILDINGS[id],lvl=this.game.state.buildings[id]??0,target=nextBuildingLevel(this.game.state,id);const h=d.production?286:260;const p=panel(350,h,.96);this.detail.addChild(p);const title=text(`${d.name} · Lv. ${lvl}`,20);title.position.set(18,16);p.addChild(title)
  ...
  if(d.levels[target]){const req=buildingRequirements(this.game.state,id,target);const cost=d.levels[target].cost;const c=text(`MEAT ${fmt(cost.meat)}   WOOD ${fmt(cost.wood)}\nCOAL ${fmt(cost.coal)}   IRON ${fmt(cost.iron)}`,12,0xffffff);c.position.set(18,y);p.addChild(c);y+=48;const rq=text(req.length?`Requires: ${req.join(' · ')}`:'All requirements met',11,req.length?0xffb0a0:0xa8e7b8);rq.position.set(18,y);p.addChild(rq);y+=42;const up=button(lvl===0?'CONSTRUCT':'UPGRADE',142,44);up.position.set(18,y);up.on('pointertap',()=>this.game.upgradeBuilding(id));p.addChild(up);const close=button('CLOSE',120,44,0x536b76);close.position.set(180,y);close.on('pointertap',()=>this.detail.visible=false);p.addChild(close)}else{const close=button('CLOSE',120,44,0x536b76);close.position.set(18,y+20);close.on('pointertap',()=>this.detail.visible=false);p.addChild(close)}this.layout()
 }
 openResearch(){
