...
import { buildingStageForLevel } from '../data/buildingProgression.js'
...
function marker(label,color=0x1d5265){const c=new Container(),g=new Graphics().circle(0,0,20).fill({color,alpha:.96}).stroke({color:0xffffff,alpha:.85,width:2}),t=txt(label,13,0xffffff,'900');t.anchor.set(.5);t.position.set(0,-1);c.addChild(g,t);return c}
function badge(label,color=0x102c37){const c=new Container(),g=new Graphics().roundRect(-34,-10,68,20,10).fill({color,alpha:.94}).stroke({color:0xffffff,alpha:.35,width:1}),t=txt(label,9,0xffffff,'900');t.anchor.set(.5);c.addChild(g,t);return c}
...
export class CityScene extends Container{
 ...
 makeTerrain(){
  ...
  for(let gy=0;gy<24;gy++)for(let gx=0;gx<24;gx++){const p=iso(gx,gy),river=gx+gy>38,bridge=river&&(gx===12||gy===12),isRoad=road.has(`${gx},${gy}`)&&!river,pool=bridge?visualAssets.bridges:river?visualAssets.water:isRoad?visualAssets.paths:visualAssets.snow,tex=pool[Math.abs((gx*7+gy*11))%pool.length],s=new Sprite(tex);s.anchor.set(.5);s.width=TW;s.height=bridge?TH*2:TH;s.position.set(p.x,p.y);s.zIndex=-5000+Math.round(p.y);if(!river&&!isRoad)s.tint=((gx+gy)&1)?0xf2f7f6:0xe8f2f2;this.addChild(s)}
 }
 ...
 makeForest(){const blocked=new Set(Object.values(LAYOUT).map(([x,y])=>`${x},${y}`));for(let i=0;i<74;i++){let gx=Math.floor(rng(i+2)*24),gy=Math.floor(rng(i+99)*24);if(gx>4&&gx<20&&gy>4&&gy<20&&rng(i+31)>.34)continue;if(blocked.has(`${gx},${gy}`)||gx+gy>38)continue;const p=iso(gx,gy),tex=visualAssets.trees[i%visualAssets.trees.length],s=sprite(tex,78+(i%5)*9);s.position.set(p.x+(rng(i+50)-.5)*48,p.y+24);s.zIndex=Math.round(s.y)-120;s.alpha=.94;this.addChild(s)}for(let i=0;i<26;i++){const gx=5+Math.floor(rng(i+310)*14),gy=5+Math.floor(rng(i+510)*14);if(blocked.has(`${gx},${gy}`))continue;const p=iso(gx,gy),s=sprite(visualAssets.props[i%visualAssets.props.length],36+(i%3)*8);s.position.set(p.x+(rng(i+700)-.5)*42,p.y+18);s.zIndex=Math.round(s.y)-30;s.alpha=.86;this.addChild(s)}}
 ...
 renderBuilding(id,c){
  ...
  const width=id==='furnace'?282:(id.includes('Mine')?160:(def.category==='military'?150:156)),art=sprite(texture,width);art.label='asset';c.addChild(art)
  if(id!=='furnace'){const overlayTex=visualAssets.buildingStages[id]?.[buildingStageForLevel(level)];if(overlayTex){const overlay=sprite(overlayTex,width);overlay.label='stageAsset';c.addChild(overlay)}}
  const levelMark=badge(id==='furnace'?`Lv.${level} · CORE`:`Lv.${level}`);levelMark.position.set(0,id==='furnace'?30:18);c.addChild(levelMark)
  if(id==='furnace'){const title=txt('FURNACE',15,0xffdf8a,'900');title.anchor.set(.5);title.position.set(0,-300);c.addChild(title);const tier=txt(furnaceTier(level).toUpperCase(),9,0xcde8ee,'900');tier.anchor.set(.5);tier.position.set(0,-278);c.addChild(tier)}
 ...
 constructionOverlay(id){const root=new Container();root.position.set(0,id==='furnace'?-250:-205);const bg=new Graphics().roundRect(-74,0,148,38,13).fill({color:0x0c2631,alpha:.96}).stroke({color:0xffd36a,width:2}),rail=new Graphics().roundRect(-58,25,116,6,3).fill({color:0x3a5059}),fill=new Graphics(),label=txt('CONSTRUCTING',9,0xffd36a,'900');label.anchor.set(.5);label.position.set(0,10);root.addChild(bg,rail,fill,label);root.label='construction';return root}
 ...
 animate(){for(const s of this.snow){s.view.y+=s.speed*2;s.view.x+=s.drift;if(s.view.y>690){s.view.y=-660;s.view.x=-900+Math.random()*1800}}const furnace=this.buildingViews.get('furnace');if(furnace){for(let i=0;i<this.smoke.length;i++){const s=this.smoke[i],phase=(performance.now()*.00022+s.phase)%1;s.view.position.set(furnace.x+35+Math.sin(phase*8+i)*7,furnace.y-230-phase*165);s.view.alpha=(1-phase)*.22;s.view.scale.set(.7+phase*1.4)}}if(this.game.state.construction){const c=this.buildingViews.get(this.game.state.construction.id),o=c?.getChildByLabel?.('construction');if(o){const j=this.game.state.construction,total=Math.max(1,j.finishAt-j.startedAt),p=Math.max(0,Math.min(1,1-(j.finishAt-Date.now())/total)),fill=o.children[2];fill.clear().roundRect(-58,25,116*p,6,3).fill(0xffcc55);o.children[3].text=`CONSTRUCTING ${Math.round(p*100)}%`}}const key=this.statusKey();if(key!==this.lastStatusKey)this.refresh();this.sortChildren()}
}
