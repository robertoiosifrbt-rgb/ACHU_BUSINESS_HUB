...
export class WorldScene extends Container{
 ...
 renderTile(def,c){c.removeChildren();const w=this.game.state.world,scouted=w.scouted.includes(def.id),owned=w.owned.includes(def.id),defeated=w.defeated.includes(def.id),burning=(w.settlementCooldown?.[def.id]??0)>Date.now(),rally=w.rally?.target===def.id;const tex=scouted?visualAssets.snow[(def.x*5+def.y*7)%visualAssets.snow.length]:visualAssets.snow[0];const ground=new Sprite(tex);ground.anchor.set(.5);ground.width=TW;ground.height=TH;ground.tint=scouted?(owned?0xeaf6ef:0xd8e7ea):0x243944;ground.alpha=scouted?1:.72;c.addChild(ground)
  if(owned)c.addChild(new Graphics().poly([-TW/2,0,0,-TH/2,TW/2,0,0,TH/2]).stroke({color:0x65d69a,width:3,alpha:.9}))
  if(rally)c.addChild(new Graphics().circle(0,-4,31).stroke({color:0xffd36a,width:4,alpha:.95}))
  ...
  if(def.kind==='settlement'){const s=fit(visualAssets.buildings.furnace,82);s.tint=burning?0xe07755:0xb98578;s.position.set(0,14);c.addChild(s);const name=txt(`[${def.tag}] ${def.name}`,7,burning?0xffb080:0xffd3ca);name.anchor.set(.5);name.position.set(0,-47);c.addChild(name);const power=txt(burning?'BURNING':`${def.strength}`,7,burning?0xff8b61:0xffa8a0);power.anchor.set(.5);power.position.set(0,25);c.addChild(power)}
